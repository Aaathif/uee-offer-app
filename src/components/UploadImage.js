...
export default function UploadImage() {
    ...
    try {
      ...
      if (status !== 'granted') {
        ...
        if (!result.canceled) {
          ...
          console.log("pickImageAsync url: ", result.assets[0].uri);
  ...
  const createFormData = (uri) => {
    console.log('createFormData() => started');
    console.log("createFormData url: ", uri);
    ...
    console.log('createFormData() => end');
    return formData;
  };

  const postImage = async ({ uri }) => {
    console.log('postImage() => started');
    console.log("postImage url: ", uri);
    const data = createFormData(uri);
    const res = await axios.post('http://192.168.8.106:3000/images', data, {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'multipart/form-data',
      },
    });
    console.log('postImage() => end');
